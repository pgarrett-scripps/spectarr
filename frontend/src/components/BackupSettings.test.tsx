...
it('retains unsaved policy edits through a failed poll and recovery', async () => {
  vi.mocked(api.backups).mockResolvedValue(base)
  vi.useFakeTimers()
  render(<BackupSettings />)
  await act(async () => {})
  fireEvent.change(screen.getByLabelText('Keep newest backups'), { target: { value: '7' } })
  vi.mocked(api.backups).mockRejectedValueOnce(new Error('Temporary outage'))
  await act(async () => { vi.advanceTimersByTime(5000) })
  expect(screen.getByLabelText('Keep newest backups')).toHaveValue(7)
  ...
  expect(screen.getByRole('button', { name: 'Save backup policy' })).toBeEnabled()
})
