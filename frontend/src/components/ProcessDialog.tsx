...
export function ProcessDialog({ projectId, runIds, onClose }: {
  ...
  useEffect(() => {
    if (initializedProfiles.current || !enabledProfiles.length) return
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (submitting || previewing || !preview || !preview.queueCount || !scope.scopeIds.length || !recipeIds.length) return
    setSubmitting(true)
    setError(null)
    try {
  ...
  return <div className="modal-backdrop" role="presentation"><section className="modal-card process-modal" role="dialog" aria-modal="true" aria-labelledby="process-title">
    <div className="modal-header"><div><h2 id="process-title">Process mass spectrometry files</h2><p>Generate reproducible derivatives for a collection of runs.</p></div><button className="icon-button" aria-label="Close" disabled={submitting} onClick={onClose}>×</button></div>
    {queued ? <div className="process-result"><CheckCircle2 size={30} /><h3>Batch queued</h3><p>{queued.queuedCount} jobs queued, {queued.skippedCount} targets skipped.</p><ProgressBar value={queued.progress} /><div className="modal-actions"><button className="button button-secondary" onClick={onClose}>Close</button><Link className="button button-primary" to="/processing" onClick={onClose}>Open processing</Link></div></div> : <form onSubmit={event => void submit(event)}>
      ...
      <div className="modal-actions"><button type="button" className="button button-secondary" disabled={submitting} onClick={onClose}>Cancel</button><button type="submit" className="button button-primary" disabled={submitting || previewing || !preview || preview.queueCount === 0 || !recipeIds.length || !scope.scopeIds.length}><FlaskConical size={15} />{submitting ? 'Queueing' : `Queue ${preview?.queueCount ?? 0} jobs`}</button></div>
    </form>}
  </section></div>
}
