...
export function Login() {
  ...
  useEffect(() => {
    api.bootstrapStatus()
      ...
      .catch(reason => setConfigurationError(reason instanceof Error ? reason.message : 'Could not reach the MassSpec API'))
  ...
  return <main className="login-page">
    <section className="login-card">
      <div className="login-brand"><span><FlaskConical size={25} /></span><div><strong>MassSpec</strong><small>Mass spec library</small></div></div>
      <div className="login-copy"><LockKeyhole size={21} /><h1>{bootstrapRequired ? 'Create the first admin' : 'Sign in'}</h1><p>{bootstrapRequired ? 'Secure this new MassSpec instance with a local administrator account.' : 'Use your local MassSpec account.'}</p></div>
      ...
      {checking && <div className="settings-placeholder">Checking the MassSpec API</div>}
