...
export function Storage() {
  ...
  return <>
    <PageHeader title="Storage" description="Inspect the filesystem storage configured for this MassSpec instance." actions={canReclaim ? <button className="button button-danger" onClick={() => setReclaiming(true)}><Trash2 size={15} /> Clear generated files</button> : undefined} />
    ...
    <div className="storage-card-grid">
      {resource.data.map(location => {
        return <article className="storage-card" key={location.id}>
          <div className="storage-card-head"><div className="storage-large-icon">{location.kind === 's3' ? <Database /> : <HardDrive />}</div><span className="storage-kind">Managed library</span></div>
          <h2>{location.name}</h2><code>{location.path}</code>
          <dl className="storage-measures"><div><dt>Cataloged data</dt><dd>{formatBytes(location.usedBytes)}</dd></div><div><dt>Volume capacity</dt><dd>{formatBytes(location.capacityBytes)}</dd></div></dl><p className="storage-measure-note">Cataloged data is the logical size of registered files, not disk space used.</p>
    ...
    <Panel title="Storage policy" subtitle="How MassSpec places and protects files">
      <div className="policy-grid">
        ...
        <Policy icon={<Database />} title="Repairable view" text="Run manifests and the complete filesystem view can be reconstructed from MassSpec metadata." />
      </div>
    </Panel>
...
function ReclaimStorageDialog({ onClose, onReclaimed }: { onClose: () => void, onReclaimed: () => void }) {
  ...
  const reclaim = () => {
    if (working || !preview || confirmation !== 'PURGE DERIVED FILES') return
    setWorking(true)
    setError(null)
  ...
  return <div className="modal-backdrop" role="presentation"><section className="modal-card reclaim-modal" role="dialog" aria-modal="true" aria-labelledby="reclaim-title">
    <div className="modal-header"><div><h2 id="reclaim-title">Clear generated files</h2><p>Remove regenerable derivatives while preserving sources, profiles, jobs, checksums, and provenance.</p></div><button className="icon-button" aria-label="Close" disabled={working} onClick={onClose}>×</button></div>
    {complete ? <div className="process-result"><CheckCircle2 size={30} /><h3>Generated files cleared</h3><p>{complete.artifactCount} files marked as purged, representing {formatBytes(complete.reclaimableBytes)} of logical data. They can be generated again from their source files.</p><button className="button button-primary" onClick={onClose}>Done</button></div> : <div className="modal-fields">
      {projects.error && <ApiErrorBanner message={projects.error} onRetry={projects.refresh} />}
      <label><span>Project</span><select disabled={working} value={projectId} onChange={event => {
      ...
      <fieldset className="choice-fieldset"><legend>Generated formats</legend>{(['mzML', 'MGF', 'MS2', 'mzXML'] as ConversionFormat[]).map(format => <label className="check-field" key={format}><input type="checkbox" disabled={working} checked={formats.has(format)} onChange={() => toggle(format)} /><span>{format}</span></label>)}</fieldset>
      ...
      <div className="modal-actions"><button className="button button-secondary" disabled={working} onClick={onClose}>Cancel</button><button className="button button-danger" disabled={working || !preview || preview.artifactCount === 0 || confirmation !== 'PURGE DERIVED FILES'} onClick={reclaim}>{working ? 'Clearing' : 'Clear generated files'}</button></div>
    </div>}
  </section></div>
}
