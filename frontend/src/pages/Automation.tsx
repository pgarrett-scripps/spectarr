...
export function Automation() {
  ...
  return <>
    <PageHeader title="Automation" description="Define desired derivatives and metadata for new and existing source files." actions={<><button className="button button-secondary" onClick={() => {
      rules.refresh()
      profiles.refresh()
    }}><RotateCw size={15} /> Refresh</button><button className="button button-primary" disabled={profiles.loading || Boolean(profiles.error)} onClick={() => {
      setError(null)
      setScope('global')
      setExtractMetadata(true)
      const standard = profiles.data.find(profile => profile.enabled && profile.name === 'Standard mzML')
      setSelectedProfiles(new Set(standard ? [standard.id] : []))
      setCreating(true)
    ...
    {creating && <div className="modal-backdrop" role="presentation"><section className="modal-card" role="dialog" aria-modal="true" aria-labelledby="new-rule-title">
      <div className="modal-header"><div><h2 id="new-rule-title">New automation rule</h2><p>The same policy applies to future sources and existing matching runs.</p></div><button className="icon-button" aria-label="Close" disabled={submitting} onClick={() => setCreating(false)}>×</button></div>
...
function ProfileEditor({ profile, onClose, onSaved }: { profile?: ProcessingProfile, onClose: () => void, onSaved: () => void }) {
  ...
  const [centroid, setCentroid] = useState(profile?.parameters.filters.some(managedPeakFilter) ?? false)
  const [ms2Only, setMs2Only] = useState(profile?.parameters.filters.some(managedLevelFilter) ?? false)
  const [submitting, setSubmitting] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const save = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const parameters: ProcessingProfile['parameters'] = {
      preset,
      filters: preset ? [] : [
        ...(profile?.parameters.filters.filter(filter => !managedPeakFilter(filter) && !managedLevelFilter(filter)) ?? []),
  ...
  return <div className="modal-backdrop" role="presentation"><section className="modal-card" role="dialog" aria-modal="true" aria-labelledby="profile-editor-title"><div className="modal-header"><div><h2 id="profile-editor-title">{profile ? `Edit ${profile.name}` : 'New processing profile'}</h2><p>Changing conversion settings creates a new profile revision.</p></div><button className="icon-button" aria-label="Close" disabled={submitting} onClick={onClose}>×</button></div><form onSubmit={event => void save(event)}><fieldset className="modal-fields form-fields" disabled={submitting}>
    ...
    <label><span>Output format</span><select disabled={Boolean(preset) || submitting} value={format} onChange={event => setFormat(event.target.value as ConversionFormat)}><option value="mzML">mzML</option><option value="MGF">MGF</option><option value="MS2">MS2</option><option value="mzXML">mzXML</option></select></label>
    {preset ? <div className="scope-summary"><strong>Settings managed by MSCLI</strong><span>MassSpec will execute the packaged named config and record its name with the profile revision.</span></div> : <><div className="field-grid"><label><span>m/z precision</span><select name="mzPrecision" defaultValue={profile?.parameters.mzPrecision ?? 64}><option value="64">64 bit</option><option value="32">32 bit</option></select></label><label><span>Intensity precision</span><select name="intensityPrecision" defaultValue={profile?.parameters.intensityPrecision ?? 32}><option value="32">32 bit</option><option value="64">64 bit</option></select></label></div>
  ...
  </fieldset>{error && <div className="modal-error" role="alert">{error}</div>}<div className="modal-actions"><button type="button" className="button button-secondary" disabled={submitting} onClick={onClose}>Cancel</button><button type="submit" className="button button-primary" disabled={submitting}>{submitting ? 'Saving' : 'Save profile'}</button></div></form></section></div>
}
...
function managedPeakFilter(filter: Record<string, unknown>) {
  return filter.kind === 'peak_picking' && filter.algorithm === 'vendor' && JSON.stringify(filter.ms_levels) === '[1,2]'
}

function managedLevelFilter(filter: Record<string, unknown>) {
  return filter.kind === 'ms_level' && JSON.stringify(filter.levels) === '[2]'
}
